...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (type === 'admin_search') {
      ...
      // 履歴取得
      const { data: histories } = await supabase.from('histories').select(`created_at, codes (アクティベーションコード, タイトル(jp))`).eq('user_id', targetUser.id).order('created_at', { ascending: false });

      const historyData = (histories || []).map(h => ({
        code: h.codes ? h.codes["アクティベーションコード"] : "不明",
        title: h.codes ? h.codes["タイトル(jp)"] : "不明なコンテンツ",
